...
| [Reading the memory map](reading_the_memory_map/README.md) | 201 | Ghidra's Memory Map on one program built as a Windows PE, an ELF and a Mach-O: where each row's bytes came from, the mebibyte of zeros no file holds and the three ways a loader records it, the W and Volatile checkboxes the decompiler obeys, an overlay that stays put when the image base moves — and the two rows in the help's own picture that Ghidra 12.1.3 no longer makes |
